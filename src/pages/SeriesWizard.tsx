...
Focus only on the overall series structure.`;
      ...
      if (quickOutput.bookOutline) {
        ...
        if (outlineData) {
          ...
          if (parsedChapters.length > 0) {
            ...
            // Create scenes for ALL chapters
            if (chapterRows && chapterRows.length > 0) {
              const allSceneInserts: Array<{
                project_id: string; chapter_id: string; title: string;
                description: string; content: string; order_index: number; status: string;
              }> = [];

              for (let chIdx = 0; chIdx < chapterRows.length; chIdx++) {
                const chRow = chapterRows[chIdx];

                if (chIdx === 0 && quickOutput.scenes) {
                  // Chapter 1: use detailed scene breakdown from Step 6
                  const parsedScenes = parseScenesFromOutput(quickOutput.scenes);
                  if (parsedScenes.length > 0) {
                    for (let si = 0; si < parsedScenes.length; si++) {
                      allSceneInserts.push({
                        project_id: currentProjectId,
                        chapter_id: chRow.id,
                        title: parsedScenes[si].title,
                        description: parsedScenes[si].description,
                        content: parsedScenes[si].content,
                        order_index: si,
                        status: 'draft',
                      });
                    }
                  } else {
                    // Fallback: create 4 placeholder scenes
                  ...
                  // Chapters 2+: parse scene stubs from chapter briefs or create 4 scaffolds
                  const chapterScenes = parseScenesFromBrief(quickOutput.chapterBriefs || '', chIdx + 1);
                  if (chapterScenes.length > 0) {
                    for (let si = 0; si < chapterScenes.length; si++) {
                      allSceneInserts.push({
                        project_id: currentProjectId,
                        chapter_id: chRow.id,
                        title: chapterScenes[si].title,
                        description: chapterScenes[si].description,
                        content: '',
                        order_index: si,
                        status: 'draft',
                      });
                    ...
                    // Default: 4 scenes per chapter as scaffolds
                    const chSummary = chRow.summary || '';
                    for (let si = 0; si < 4; si++) {
                      allSceneInserts.push({
                        project_id: currentProjectId,
                        chapter_id: chRow.id,
                        title: `Scene ${si + 1}`,
                        description: si === 0 ? `Opening scene. ${chSummary.slice(0, 200)}` : '',
                        content: '',
                        order_index: si,
                        status: 'draft',
                      });
              ...
              console.log('[Wizard Save] Total scenes to insert:', allSceneInserts.length);
              // Insert in batches to avoid payload limits
              const batchSize = 50;
              for (let i = 0; i < allSceneInserts.length; i += batchSize) {
                const batch = allSceneInserts.slice(i, i + batchSize);
                await supabase.from('scenes').insert(batch);
              }
              console.log('[Wizard Save] All scenes inserted successfully');
            }
          } else {
  ...
  // Parse scene titles from chapter briefs for a specific chapter number
  function parseScenesFromBrief(briefsText: string, chapterNum: number): Array<{ title: string; description: string }> {
    if (!briefsText) return [];

    // Find the section for this chapter
    const lines = briefsText.split('\n');
    let inChapter = false;
    let inScenes = false;
    const scenes: Array<{ title: string; description: string }> = [];

    for (const line of lines) {
      const trimmed = line.trim();

      // Detect chapter header (e.g., "**Chapter 2:", "Chapter 2 -", "### Chapter 2")
      const chMatch = trimmed.match(/(?:\*{2})?(?:#+\s*)?Chapter\s+(\d+)/i);
      if (chMatch) {
        ...
        if (num === chapterNum) {
          ...
          break; // We've passed our chapter
        }
        continue;
      }
      ...
      // Detect scenes section header
      if (/^(?:#+\s*)?(?:\*{2,})?Scenes/i.test(trimmed) || /^#{1,4}\s*Scenes/i.test(trimmed)) {
        inScenes = true;
        continue;
      }

      // Stop at next section header within the chapter
      if (inScenes && /^(?:#+\s*)?(?:\*{2,})?(?:Character|Key Dialogue|Closing|Opening|Theme)/i.test(trimmed)) {
        inScenes = false;
        continue;
      }

      // Parse numbered scene lines (e.g., "1. Benjamin meets Kate" or "- Scene: ...")
      if (inScenes || inChapter) {
        const numberedMatch = trimmed.match(/^(\d+)\.\s+(.+)/);
        if (numberedMatch) {
          const desc = numberedMatch[2].trim();
          scenes.push({
            title: `Scene ${numberedMatch[1]} - ${desc.slice(0, 60)}`,
            description: desc,
          });
        }
      }
              ...
              <div className="flex items-center gap-2">
                ...
                {(job.status === 'completed' || job.status === 'cancelled' || job.status === 'failed') && (
                  <button
                    ...
                    className="px-2 py-0.5 text-[10px] bg-slate-50 text-slate-500 rounded hover:bg-slate-100 transition-colors"
                  >
                    Remove
                  </button>
                )}
              ...
              <button
                onClick={async () => {
                  ...
                  if (idsToDelete.length > 0) {
                    await supabase.from('generation_jobs').delete().in('id', idsToDelete);
                    loadJobs();
                  }
                }}
                className="px-3 py-1 text-xs text-slate-500 hover:text-slate-700 hover:bg-slate-50 rounded transition-colors"
              >
                Clear All Finished
              </button>
            </div>
