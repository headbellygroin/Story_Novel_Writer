...
function injectLipsyncParams(
  ...
  for (const nodeId of Object.keys(w)) {
    ...
    if (typeof inputs.image === 'string') {
      inputs.image = characterImageUrl;
    }
    if (typeof inputs.image_path === 'string') {
      inputs.image_path = characterImageUrl;
    }
    if (typeof inputs.face_image === 'string') {
      inputs.face_image = characterImageUrl;
    }
    if (typeof inputs.reference_image === 'string') {
      inputs.reference_image = characterImageUrl;
    }

    if (typeof inputs.audio === 'string') {
      inputs.audio = audioUrl;
    }
    if (typeof inputs.audio_path === 'string') {
      inputs.audio_path = audioUrl;
    }
    if (typeof inputs.audio_file === 'string') {
      inputs.audio_file = audioUrl;
    }
    if (typeof inputs.driven_audio === 'string') {
      inputs.driven_audio = audioUrl;
    }
  }
...
export async function generateLipsync(
  ...
  const workflow = injectLipsyncParams(settings.workflow, characterImageUrl, audioUrl);
...
function waitForLipsyncResult(
  ...
  return new Promise((resolve, reject) => {
    ...
    const overallTimeout = setTimeout(() => {
      if (!settled) {
        cleanup();
        reject(new Error('Lip-sync generation timed out after 15 minutes'));
      }
    }, timeoutMs);
...
async function pollLipsyncFallback(endpoint: string, promptId: string): Promise<string> {
  ...
  throw new Error('Lip-sync generation timed out after 15 minutes');
}
